...
import {View, TouchableOpacity, Image, ScrollView, FlatList, TextInput, Text, ActivityIndicator, Alert, Platform, Button, StyleSheet} from 'react-native';
    ...
    return (
      <TouchableOpacity onPress={() => this.props.navigation.navigate('MoviesByLanguages', {HeaderName: item.language})} key={item.language} style={styles.renderLanguages}>
        <View style={{height: '100%', width: '100%', justifyContent: 'flex-end', alignItems: 'center', borderRadius: 5}}>
          <Image style={{height: '100%', width: '100%', justifyContent: 'flex-end', alignItems: 'center', borderRadius: 5}} source={{uri: item.image}} imageStyle={{borderRadius: 6}} blurRadius={50}  contentFit={'cover'}></Image>
